...
{
    public class AffixArmored : BaseEliteAffix
    {
        ...
        {
            base.PreLoad();
            equipmentDef.name = "AffixArmored";
        ...
        public override void OnLoad()
        {
            base.OnLoad();
            SetAssets("Armored");
            AdjustElitePickupMaterial(new Color32(229, 172, 172, 255), 4f);
            ...
            selfBuffUseEffect = Main.AssetBundle.LoadAsset<GameObject>("Assets/EliteVariety/Elites/Armored/SelfBuffInflictSFX.prefab");
            ...
            effectComponent.soundName = "EliteVariety_Play_item_use_affixarmored";
        ...
        public override void AspectAbilitiesSupport()
        {
            AspectAbilities.AspectAbilitiesPlugin.RegisterAspectAbility(new AspectAbilities.AspectAbility
            {
                ...
                onUseOverride = (equipmentSlot) =>
                {
                    ...
                    equipmentSlot.characterBody.AddTimedBuff(EliteVarietyContent.Buffs.ArmoredSelfBuff, 10f);
                    return true;
                }
            });
        }
        ...
        {
            orig(self);
            self.AddItemBehavior<EliteVarietyAffixArmoredEquipmentBehavior>(self.inventory.currentEquipmentIndex == equipmentDef.equipmentIndex ? 1 : 0);
        }

        public class EliteVarietyAffixArmoredEquipmentBehavior : CharacterBody.ItemBehavior
        {
            public void Awake()
            {
                shieldProgressBarList = new List<List<Renderer>>();
            ...
            public void Start()
            {
                ...
                if (component)
                {
                    ...
                    if (modelTransform)
                    {
                        ...
                        if (characterModel)
                        {
                            foreach (CharacterModel.ParentedPrefabDisplay parentedPrefabDisplay in characterModel.parentedPrefabDisplays.FindAll(x => x.equipmentIndex == EliteVarietyContent.Equipment.AffixArmored.equipmentIndex))
                            {
                                Transform progressBarTransform = parentedPrefabDisplay.instance.transform.Find("ShieldProgressBar");
                                if (progressBarTransform)
                                {
                                    List<Renderer> shieldProgressBar = new List<Renderer>();
                                    foreach (Renderer renderer in progressBarTransform.gameObject.GetComponentsInChildren<Renderer>())
                                    {
                                        shieldProgressBar.Add(renderer);
                                    }
                                    shieldProgressBarList.Add(shieldProgressBar);
                                }
                            }
                        }
                    }
            ...
            {
                float progress = body.healthComponent.Networkbarrier / Mathf.Max(body.maxBarrier, 1f);

                foreach (List<Renderer> shieldProgressBar in shieldProgressBarList)
                {
                    for (var i = 0; i < shieldProgressBar.Count; i++)
                    {
                        Renderer renderer = shieldProgressBar[i];

                        renderer.GetPropertyBlock(materialPropertyBlock);
                        materialPropertyBlock.SetFloat("_EmPower", 8f * Mathf.Clamp01((progress - ((float)i / (float)shieldProgressBar.Count)) / (1f / (float)shieldProgressBar.Count)));
                        renderer.SetPropertyBlock(materialPropertyBlock);
                    }
                }
            }

            public List<List<Renderer>> shieldProgressBarList;
